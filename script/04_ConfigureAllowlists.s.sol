...
contract ConfigureAllowlists is Script {
    function run() external {
        ...
        address devBuyV3 = vm.envOr("LIQUID_UNIV3_ETH_DEV_BUY", address(0)); // absent on V4-only chains
        ...
        if (devBuyV3 != address(0)) {
            liquid.setExtension(devBuyV3, true);
            console.log("Enabled extension:", devBuyV3);
        } else {
            console.log("V3 dev-buy: SKIPPED (not deployed on this chain)");
        }
