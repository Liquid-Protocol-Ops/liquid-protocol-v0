...
contract DeployAndWireAdapters is Script {
    ...
    function run() external {
        ...
        _execSafe(VAULT, abi.encodeWithSignature(
            "setVenueAdapter(address,bool)", address(antSeed), true
        ));
        console.log("Registered AntSeedAdapter");

        _execSafe(VAULT, abi.encodeWithSignature(
            "setVenueAdapter(address,bool)", address(surplus), true
        ));
        console.log("Registered SurplusAdapter");

        _execSafe(VAULT, abi.encodeWithSignature(
            "setVenueAdapter(address,bool)", address(x402), true
        ));
