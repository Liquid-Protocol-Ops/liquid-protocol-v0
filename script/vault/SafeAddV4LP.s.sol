...
contract LiquidityHelper {
    ...
    int24 constant TICK_LOWER = 62160;   // ~$500/ETH
    int24 constant TICK_UPPER = 92100;   // ~$10,000/ETH
...
contract SafeAddV4LP is Script {
    ...
    function run() external {
        ...
        _execSafe(
            WETH,
            abi.encodeWithSignature("transfer(address,uint256)", address(helper), WETH_BUDGET)
        );
        ...
        _execSafe(
            address(helper),
            abi.encodeWithSignature("addLiquidity(uint128)", LIQUIDITY)
        );
