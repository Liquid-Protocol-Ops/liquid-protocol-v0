...
forge build --profile lplocker                       # LiquidLpLockerFeeConversion only (100 runs — too big at 20k)
...
| `oracles/` | Morpho Blue price oracles, one per market: `WstDiemUsdcOracle`, `WstDiemWethOracle` (both **hardcode `DIEM = $1`** — see Security & Audit), and `WstDiemVvvOracle` (fully on-chain, no USD feed — `convertToAssets()` × Aerodrome DIEM/VVV TWAP, immutable; the MOG-544 fix). |
| `WstDIEMHook` | Uniswap V4 hook for the wstDIEM/WETH pool — intended to apply a dynamic LP fee on NAV deviation. **Currently a stub** that always returns `FEE_NORMAL` (5 bps); full TWAP-oracle fee logic is deferred to WP-5. |
