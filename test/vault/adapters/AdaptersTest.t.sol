...
contract AdaptersTest is Test {
    ...
    function setUp() public {
        ...
        vm.prank(settler); mockUsdc.approve(address(antSeed), type(uint256).max);
        vm.prank(settler); mockUsdc.approve(address(surplus),  type(uint256).max);
        vm.prank(settler); mockUsdc.approve(address(x402),     type(uint256).max);
    }
    ...
    function test_x402_recordX402Settlement_permissionless() public {
        ...
        vm.prank(payer); mockUsdc.approve(address(x402), type(uint256).max);
        vm.prank(payer); x402.recordX402Settlement(10e6);
    ...
    function test_receiveSettlement_revertsUnauthorized() public {
        ...
        vm.prank(alice); mockUsdc.approve(address(antSeed), 100e6);
        vm.prank(alice);
    ...
    function test_routeYield_raisesHolderRate() public {
        _giveUsdc(address(antSeed), 1_000e6);
        uint256 rateBefore = vault.convertToAssets(1e18);
        vm.prank(keeper); antSeed.routeYield();
    ...
    function test_surplus_routeYield_works() public {
        ...
        uint256 rateBefore = vault.convertToAssets(1e18);
        vm.prank(keeper); surplus.routeYield();
    ...
    function test_x402_routeYield_works() public {
        ...
        uint256 rateBefore = vault.convertToAssets(1e18);
        vm.prank(keeper); x402.routeYield();
        assertGt(vault.convertToAssets(1e18), rateBefore);
    }
    ...
    function test_routeYield_allowsOwner() public {
        _giveUsdc(address(antSeed), 100e6);
        vm.prank(owner); antSeed.routeYield();
    }
    ...
    function test_setOperatorFeeBps_succeedsAtMax() public {
        vm.prank(owner); antSeed.setOperatorFeeBps(2000);
    ...
    function test_inferenceName_antSeed() public view { assertEq(antSeed.inferenceName(), "AntSeed"); }
    function test_inferenceName_surplus() public view { assertEq(surplus.inferenceName(), "Surplus Intelligence"); }
    function test_inferenceName_x402()    public view { assertEq(x402.inferenceName(),    "X402"); }
